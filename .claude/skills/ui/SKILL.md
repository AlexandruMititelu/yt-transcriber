...
- Use design tokens only: `--accent`, `--accent-soft`, `--surface`, `--surface-2`, `--border`, `--muted`, `--text`, `--radius`, `--radius-sm`, `--green`, `--green-soft`, `--font-ui`, `--font-mono`. Never hard-code colours except the shared yellow highlight `#ffd60a` and destructive red `#ff453a`.
